(function () {
  ...
  function enviarLoteMutacoes(lote) {
    ...
    call('salvarMutacoesDashboard',{requestId:lote.requestId,patches:lote.entries.map(function(entry){return entry.patch;})}).then(function(){if(lote.entries.some(function(entry){return entry.patch.tipo==='fluxoChurn'||entry.patch.tipo==='excluirFluxoChurn';}))state.churnAnalyticsCache=Object.create(null);setSave('Salvo');agendarSincronizacaoDeFundo();}).catch(function(){
      lote.entries.slice().reverse().forEach(function(entry){reverterMutacaoOtimista(entry.rollback);});state.failedMutations.push(lote);setSave('Não foi possível salvar. Tentar novamente.');setRetryVisible(true);
    }).finally(function(){state.saving=false;if(state.mutationQueue.length)flushQueue();});
  }
  function enqueue(patch) { var entry={patch:patch,rollback:aplicarMutacaoOtimista(patch)};state.mutationQueue.push(entry);setSave('Alteração pendente');flushQueue(); }
